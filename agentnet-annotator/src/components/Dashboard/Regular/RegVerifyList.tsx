...
export default function RegVerifyList() {
    ...
    const filteredRows: onlineRecordingProp[] = allVerifyTasks.filter(
        (toVerifyTask: onlineRecordingProp) => {
            const nameOrId =
                toVerifyTask.task_name === undefined ||
                toVerifyTask.task_name === ""
                    ? toVerifyTask.recording_id
                    : toVerifyTask.task_name;

            return (
                nameOrId.toLowerCase().includes(searchTerm.toLowerCase()) &&
                (toVerifyTask.status === selectedStatus ||
                    selectedStatus === null)
            );
        }
    );
