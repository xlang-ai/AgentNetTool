...
const AdminChart = ({ data }: MyChartProps) => {
    const [filter, setFilter] = useState<string>('all');
    const [userFilter, setUserFilter] = useState<string>('all');
    const { fetchStats, userDict } = useMain();
    // 根据筛选条件过滤数据
    ...
    return (
        <div style={{ width: '100%' }} className="flex flex-col items-center">
            <div style={{ width: '95%' }} className="flex justify-between items-center grid grid-cols-5 gap-4">
                <p className="text-2xl font-bold col-span-3">Statistics Overview</p>
                <FormControl size="sm" className="col-span-1 w-full">
                    ...
                    >
                        ...
                        {Object.entries(userDict).map(([id, name]) => (
            ...
            <div key={filter} style={{ width: '100%', height: "20vh" }}>
                <ResponsiveContainer>
                    <AreaChart
                        data={filteredData}
                        margin={{
